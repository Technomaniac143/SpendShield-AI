...
Deterministic business logic beats LLM reasoning for financial calculations.
Evidence beats assertions — every AI conclusion must reference evidence.
PostgreSQL is the transactional source of truth.
Neo4j is for relationships, not financial authority.
Blockchain (Hyperledger Fabric) verifies evidence provenance only — never stores
business documents on-chain, never used for ordinary analytics.
AI recommends; humans approve high-impact actions (human-in-the-loop).
Every financial number must have a traceable calculation source.
Tenant isolation is mandatory everywhere (API, service, DB, graph, storage, cache).
Potential savings must never be presented as realized savings.
The backend must remain useful even when the LLM is unavailable.
---
...
AI investigation orchestration (AuditAgent)
Evidence-grounded explanations
True-cost procurement engine
Financial exposure calculation
Inventory-to-cash intelligence
Savings estimation
Action recommendation engine
Human-in-the-loop decisions
Blockchain evidence verification
SHA-256 document integrity verification
Multi-party procurement event provenance
Outcome tracking
Outcome-adjusted learning
Audit trail
RBAC
...
`auth, dashboard, suppliers, transactions, invoices, procurement, anomalies, investigations, graph, true-cost, exposure, inventory, recommendations, evidence, outcomes, health` — see `docs/backend-prompt.md` §41 for exact routes.
...
User: "Investigate ABC Industries." → AuditAgent starts investigation.
Supplier analysis → `risk = 87`, `confidence = 93%`.
Transaction analysis → 80-unit GRN mismatch, ₹40,000 exposure.
Duplicate detection → 3 potential near-duplicate invoices.
Price analysis → 11.6% price deviation.
Supplier performance → 12% delivery delay, 6.2% defect rate.
Graph → 2 relationship risk signals.
True cost → ABC Industries ₹1,105 vs Supplier B ₹1,060.
Recommendation → Hold affected invoice / Investigate supplier / Shift future volume.
Evidence → SHA-256 → Hyperledger Fabric → VERIFIED.
Human decision → ACCEPT.
Outcome recorded → predicted savings, actual savings, cash released.
---
